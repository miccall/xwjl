...
namespace FluffyUnderware.Curvy
{
    ...
    public class FollowSpline : CurvyComponent
    {
        ...
        /// </summary>
        [Label("Use 2D Orientation","Use 2D Orientation (along z-axis only)?")]
        ...
        protected void UpdateRelative()
        {
            ...
            {
                Transform.position = Spline.MoveFast(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
            }
            else
            {
                Transform.position = Spline.Move(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
            }
        ...
        protected void UpdateAbsoluteExtrapolate()
        {
            if (FastInterpolation)
            {
                Transform.position = Spline.MoveByFast(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
            }
            else
            {
                Transform.position = Spline.MoveBy(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
            }
        ...
        {
            Transform.position = Spline.MoveByLengthFast(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
            if (SetOrientation)
                orientate();
        }
