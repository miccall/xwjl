...
namespace FluffyUnderware.DevToolsEditor
{
    public class DTEditor<T> : Editor where T : MonoBehaviour
    {
        ...
        {
            get { return (target != null && Target.transform == Selection.activeTransform) ? true : false; }
        }
        ...
        DTGroupNode mRootNode = new DTGroupNode("Root");
                    ...
                    foreach (IDTGroupParsingAttribute ga in groupParseAttribs)
                    {
                        ...
                        if (ga is TabAttribute)
                        {
                            ...
                            if (!string.IsNullOrEmpty(tabA.TabBarName) && !string.IsNullOrEmpty(tabA.TabName))
                            {
                                ...
                                Debug.LogWarningFormat("Skipping [Tab] on '{0}' because Path is missing TabBar or Tab!", iterator.propertyPath);
